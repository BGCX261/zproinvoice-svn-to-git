...
function ItemNameChanged(sCURRENCY_ID, fldNAME)
{
	...
	if ( fldPREVIOUS_NAME.value != fldNAME.value )
	{
		if ( fldNAME.value.length > 0 )
			CRM.InvoiceManagement.Products.ProductCatalog.AutoComplete.GetItemDetailsByName(GetCurrencyID(sCURRENCY_ID), fldNAME.value, ItemChanged_OnSucceededWithContext, ItemChanged_OnFailed, userContext);
	}
}
...
function ItemPartNumberChanged(sCURRENCY_ID, fldMFT_PART_NUM)
{
	...
	if ( fldLastLineItemPartNumber.value != fldMFT_PART_NUM.value )
	{
		if ( fldMFT_PART_NUM.value.length > 0 )
			CRM.InvoiceManagement.Products.ProductCatalog.AutoComplete.GetItemDetailsByNumber(GetCurrencyID(sCURRENCY_ID), fldMFT_PART_NUM.value, ItemChanged_OnSucceededWithContext, ItemChanged_OnFailed, userContext);
	}
}
