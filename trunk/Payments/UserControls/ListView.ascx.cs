    ...
    public partial class ListView : CRMControl
    {
        ...
        protected void Page_Command(object sender, CommandEventArgs e)
        {
            ...
            else if (e.CommandName == "MassUpdate")
            {
                ...
                if (arrID != null)
                {
                    Stack stk = Utils.FilterByACL_Stack(m_sMODULE, "edit", arrID, "PAYMENTS");
                    if (stk.Count > 0)
                    {
                        while (stk.Count > 0)
                        {
                            string sIDs = Utils.BuildMassIDs(stk);
                            CommonProcedure.PatmentsMassUpdate(sIDs, ctlMassUpdate.ASSIGNED_USER_ID,
                                                               TimeZone.GetTimeZone.ToServerTime(
                                                                   ctlMassUpdate.PAYMENT_DATE),
                                                               ctlMassUpdate.PAYMENT_TYPE, ctlMassUpdate.TEAM_ID);
                        }
            ...
            else if (e.CommandName == "MassDelete")
            {
                ...
                if (arrID != null)
                {
                    Stack stk = Utils.FilterByACL_Stack(m_sMODULE, "delete", arrID, "PAYMENTS");
                    if (stk.Count > 0)
                    {
